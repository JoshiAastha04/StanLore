...
-- ─────────────────────────────────────────────────────────────────────────────
-- SUMMARY: How stars are earned (call earn_stars RPC)
--
--   daily_login       +1    claim_daily_login() — once per day
--   profile_setup     +5    when user sets display_name + bio for the first time
--   first_card        +10   when collection goes from 0 → 1 card
--   era_complete      +50   when all cards in an era are owned
--   trade             +15   when a trade is marked complete (both sides)
--
-- How stars are spent:
--   catalog_purchase  -5    common card
--   catalog_purchase  -15   uncommon card
--   catalog_purchase  -30   rare card
--
-- All deductions go through spend_stars_for_card() which is atomic.
-- All earnings go through earn_stars() which logs every transaction.
-- ─────────────────────────────────────────────────────────────────────────────
